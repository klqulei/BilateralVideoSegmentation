...
%
function bilateralData = lift(vid)
...
bilateralData(:,end) = repelem(1:testcase.frames, testcase.height*testcase.width);
    
%% scale to grid size
eps = 0.001;
lBounds = min(bilateralData)-eps;
uBounds = max(bilateralData)+eps;
scaleFactors = (params.gridSize-1) ./ (uBounds - lBounds);
